...
const { apiURL, sseEnable, websocketEnable } = useAppConfig(
  import.meta.env,
...
export function useSseMessage() {
  ...
  const sseAddr = `${apiURL}/resource/sse?token=Bearer ${token}`;
...
export function useWebSocketMessage() {
  ...
  const token = accessStore.accessToken;
  // 这里是http链接形式
  let websocketAddr = `${apiUrlStr}/resource/websocket?token=Bearer ${token}`;
  // http/https处理
